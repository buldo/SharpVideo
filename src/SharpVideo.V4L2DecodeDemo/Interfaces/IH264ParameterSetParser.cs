...
public interface IH264ParameterSetParser
{
    ...
    /// <summary>
    /// Parse SPS to control structure (legacy method)
    /// </summary>
    V4L2CtrlH264Sps ParseSpsToControl(byte[] spsData);

    /// <summary>
    /// Parse PPS to control structure (legacy method)
    /// </summary>
    ...
    /// <summary>
    /// Parse slice header to create V4L2 slice parameters (byte overload)
    /// </summary>
    V4L2CtrlH264SliceParams ParseSliceHeaderToControl(byte[] sliceData, byte sliceType);

    /// <summary>
    /// Get NALU header position in data
    /// </summary>
    int GetNaluHeaderPosition(byte[] data, bool useStartCodes = true);
}